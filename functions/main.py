...
def plot_end_wave(wt, drive, nb_drive, ref_values, index_time, lambda_back, dx, s, x_graph_values, dir_save):  
    ...
    chasing = np.where(wt[-10,:]>20)[0][0]<np.where(drive[-10,:]>0)[0][0]    
    if chasing: 
...
m = 0.2
print("sigma au carré =", np.round(m*dx**2/(2*0.1),3) )
print("sigma =", np.sqrt(np.round(m*dx**2/(2*0.1),3)) )
# sigma = 0.5
# print("m = ", sigma**2 * (2*0.1)/(dx**2) )
...
dir_load = f"../../../stoch_not_save/datas/1D_{conv_timing}_K_{int(np.log10(K))}_dx_{dx}_s_{s}_r_{r}_m_{m}"
...
c = float(para_list[11].replace('c=', ''))
h = float(para_list[12].replace('h=', ''))
file.close()
...
if load: 
    ...
    wt = np.loadtxt(f"{dir_load}/nW_matrix.txt")           # Matrix wt : row = times, column = space
    drive = np.loadtxt(f"{dir_load}/nD_matrix.txt")        # Matrix drive : row = times, column = space
...
dir_save = f"../outputs/K_{int(np.log10(K))}_s_{s}_m_{m}"
...
nb_drive_around_last_wt = np.zeros(len(index_time))
for i in range(len(index_time)) : 
    nb_drive_around_last_wt[i] = drive[index_time[i], int(posi_1[0,:][i])]
print('\nNumber of drive alleles around the last wild-type allele :', np.mean(nb_drive_around_last_wt))
