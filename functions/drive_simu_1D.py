...
def erad_s(conv_timing, r, c, h, sigma) : 
    ...
    if conv_timing == "ger" : 
        ...
        if lin > 0 : print("Linear speed :", 2*np.sqrt(lin))
    return(2*np.sqrt(sigma)*np.sqrt(lin))


# /!\ r=0 empêche une vrai explosion après recolonisation !

K = 10**5        # Carrying capacity for a spatial interval of size 1
...
v_cont = erad_s(conv_timing, r, c, h, sigma) 
nb_sites = (int(v_cont*T/dx*(1+1/5)//100)+1)*100

kymo = False

# Where to save datas
dir_save = f"../../../stoch_not_save/datas/1D_{conv_timing}_K_{int(np.log10(K))}_dx_{dx}_s_{s}_r_{r}_m_{m}"
